...
import { getVisitorId, isUniqueVisit, getDeviceInfo, getGeoData } from './utils';
...
export async function recordUserAction(userId: string, actionType: string, details?: string) {
...
```typescript
export async function logUserError(userId: string, errorContext: string, errorDetails: string) {
  await recordUserAction(userId, 'ERROR', JSON.stringify({ context: errorContext, details: errorDetails }));
}
```
