...
```typescript
export const authenticatedUserActions = sqliteTable('authenticated_user_actions', {
  id: integer('id').primaryKey({ autoIncrement: true }),
...
export const siteVisitsTimestampIndex = index('site_visits_timestamp_idx').on(siteVisits.timestamp);
export const siteVisitsVisitorIdIndex = index('site_visits_visitor_id_idx').on(siteVisits.visitorId);

// Indexes for userSessions
export const userSessionsVisitorIdIndex = index('user_sessions_visitor_id_idx').on(userSessions.visitorId);
export const userSessionsStartTimeIndex = index('user_sessions_start_time_idx').on(userSessions.startTime);

// Indexes for authenticatedUserActions
export const authenticatedUserActionsUserIdIndex = index('authenticated_user_actions_user_id_idx').on(authenticatedUserActions.userId);
export const authenticatedUserActionsTimestampIndex = index('authenticated_user_actions_timestamp_idx').on(authenticatedUserActions.timestamp);
```
