...
const mockUser = {
	...
	avatar: 'https://i.pinimg.com/736x/b8/6e/7f/b86e7f92cc6ff55968a8a67b95499a82.jpg',
	role: 'admin',
	createdAt: new Date(),
	updatedAt: new Date(),
	preferences: {
...
export function Sidebar({
	...
	const { lists, activeListId, createList, setActiveList } = useListStore()
	...
	const handleEditItem = (id: string, newLabel: string, newColor: string) => {
		setNavItems(
			navItems.map((item) =>
				item.id === id
					? { ...item, label: newLabel, color: newColor }
					: item
			)
		)
	}
			...
			{!searchQuery.trim() && (
				<div className="flex-1 overflow-auto py-4">
					<div className="px-3 mb-2">
						<h3 className="text-xs font-medium text-muted-foreground mb-2 px-3">QUICK ACCESS</h3>
						<nav className="space-y-1">
							<DragDropContext onDragEnd={onDragEnd}>
								<Droppable droppableId="sidebar-list" isDropDisabled={false} isCombineEnabled={false} ignoreContainerClipping={false}>
									{(provided) => (
					...
					<div className="px-3 mb-2">
						<h3 className="text-xs font-medium text-muted-foreground mb-2 px-3">RECENT LISTS</h3>
						<nav className="space-y-1">
							{lists.slice(0, 3).map((list) => (
								<button
									...
									className={cn(
										...
										list.id === activeItem ? 'text-foreground bg-primary/5' : 'text-muted-foreground hover:text-foreground hover:bg-primary/5'
									)}
								>
